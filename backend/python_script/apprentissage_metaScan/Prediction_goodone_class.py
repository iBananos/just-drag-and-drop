...
def get_stacking():
	...
	return model
init=pd.read_csv('wrongPredictionall_class.csv')
...
X_train=X_train.drop('accuracy',axis=1)
...
# Fit the random search model
rf.fit(X_train, y_train)
filename = 'SecondaryGoodPredictor_class.sav'
pickle.dump(rf, open(filename, 'wb'))
