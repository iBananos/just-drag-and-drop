...
describe('Test des utilitaires d\'Utils', () => {
    ...
    test('Test de la méthode getDataFiles()', async() => {
        expect(Utils.default.getDataFiles("demo",'uploads/' +"demo"+ '/databaseInfo/',true)).not.toBeNull();
        expect(Utils.default.getDataFiles("demo",'uploads/' +"demo"+ '/databaseInfo/',true).length).toBe(1);
        expect(Utils.default.getDataFiles("demo",'uploads/' +"demo"+ '/databaseInfo/',true)).toEqual([{"name":"diamonds","date":"Thu Dec 16 2021 22:26:54 GMT+0100 (heure normale d’Europe centrale)","size":3192560,"extension":"csv","separator": ",","colonnes":["","carat","cut","color","clarity","depth","table","price","x","y","z"],"colonnesString":["cut","color","clarity"]}]);
    });
